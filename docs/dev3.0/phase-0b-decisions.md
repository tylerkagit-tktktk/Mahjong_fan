...
- the Phase 0B foundation schema was version `300`; Phase 1C added schema `301`, and Phase 1E supersedes it with current schema `302`;
...
- The foundation introduced the typed errors `UnsupportedOlderSchemaVersionError`, `ForwardSchemaVersionError`, and `SchemaInitializationError`, plus the explicit app-owned object manifest. Current schema version is now `302`; versions `300` and `301` are supported non-destructive migration sources.
...
- The local schema is now `302`: `300` remains the replay foundation source, `301` adds explicit seat boundaries, and `302` adds immutable local hand-revision storage. Fresh/legacy-reset databases create the complete 302 object set atomically. A `300` database migrates non-destructively through `301` and then `302`; a `301` database creates only the revision table/index. Every migration verifies its objects before its version stamp, and failures roll back with the prior version and rows intact.
