...
## Phase 2B-1 Cloud Canonical Adapter Outcome

- Cloud lineup → canonical adapter semantics are now specified and implemented as a pure domain path: runtime `unknown` is strictly recognized as `legacy_unversioned_cloud_archive_v0`, adapted to canonical initial seats/hands/boundaries, replayed by `replayGameRecord()`, and exposed through a non-UI characterization projection. No default rules, current-lineup fallback, name matching, seat guessing, partial result, clock, random ID, Firebase, or SQLite dependency is used.
- Cloud one-based completed hands validate as contiguous `1..N` before normalizing to canonical `0..N-1`; source hand ID/index trace remains outside canonical storage types. `currentVersion`, `currentHandIndex`, hand versions, and lineup versions remain distinct source metadata.
- UID and temporary-player ID remain separate identities. Canonical replay now supports at least four historical identities while every initial/effective lineup remains exactly four unique known identities; the Local adapter still rejects any local player-row count other than four. Completed-game ranking/stats include only identities present in a completed hand's exact effective lineup. A pending post-final boundary updates final seats without adding a never-played identity; a zero-hand result uses the lineup effective for Cloud Hand 1.
- Exactly one source lineup at effective index 0 supplies initial seats. Later source effective hand `k` maps to canonical boundary `k - 1`, including a valid pending boundary at `handCount`. Every hand requires its exact effective `lineupVersion`; multiple host changes for the same future index use only the highest/latest version. Missing, superseded, unknown, duplicate, incomplete, backward, or gapped lineup evidence rejects.
- Legacy Cloud implicitly starts with dealer seat 0, now recorded as an explicit compatibility rule and informational diagnostic. Per-hand dealer evidence remains `null`; canonical replay derives dealer/round truth. Cloud stores no settlement deltas, so none are fabricated. Complete supported HK RulesV1 is mandatory and malformed/unsupported rules never fall back.
- The characterization projection reuses Local competition ranking and tied-leader semantics, separates current-hand round labels from final next-hand state, and demonstrates traditional/custom settlement, totals, statistics, dealer/round, reseat, pending-seat, corruption, determinism, and long-timeline behavior. `canonicalValid` means complete replay relative to the frozen source; source trust remains `legacy_unverified`, not tamper-proof or server-authoritative.
- Fully rewritten destructive temp Merge history cannot be reconstructed when no old identity evidence remains. Detectable missing references reject; erased pre-merge identity is never invented.
- Cloud archive/detail UI, multiplayer table, Firestore rules, archive/SQLite infrastructure, local Dashboard, lifecycle, and correction remain unchanged. **Multiplayer correction remains NOT READY** until Firestore authority/lifecycle hardening, immutable mutation revisions, source/archive versioning and integrity, non-destructive merge semantics, actor/stale guards, and replay-after-write contracts are implemented.

## Phase 2B-2 Cloud Ended Result Integration Outcome

- Cloud ended results now render from the canonical Cloud replay/projection path. `CloudArchiveDetailScreen` passes the loaded local archive to `buildCloudCanonicalResult` and consumes canonical ranking, statistics, hand/current-round projections, final round, validated rules and replay-derived winner gain. Its former settlement, tolerant rules, heuristic lineup, dealer/round, total, ranking, participant and stats reducer is removed.
- A loaded archive with invalid canonical data never falls back to a plausible partial result or default HK rules. It shows a localized back-only unavailable state without raw diagnostics. Valid `legacy_unverified` archives render normally; source-trust limitations remain internal.
- Local and Cloud ended-result UX are aligned, while active-game and Cloud lifecycle behavior remain separate. Cloud now follows App Bar Share → Hero → competition Ranking → compact 2x2 Stats → one collapsed global History disclosure → one collapsed Rules card. Archive sync/host cleanup remains a secondary Cloud-only footer after result content.
- Cloud source Hand 1 now displays as Hand 1 through canonical zero-based trace. History uses canonical current-hand rounds, quiet wind grouping, event-first zimo/discard/draw rows and one winner amount; draws have no amount. Filters, per-wind disclosures, per-player stats lines, four-seat delta chips and bottom Share are removed.
- Ranking supports `1,1,3,4`, all-zero `1,1,1,1`, every actual historical participant beyond four, and pending never-played exclusion. Stats retain every tied zimo/discard leader and show `—` for zero categories. Rules use only the strict historical canonical snapshot.
- Share uses the canonical screen projection for date/title, competition ranking, balances, hands/draws and tied leaders. It has one accessible App Bar entry, a synchronous duplicate guard, normal cancellation, localized failure Alert and unmount protection.
- English, Traditional Chinese and Simplified Chinese add only canonical-result-unavailable copy; shared Local result/timeline/accessibility keys are reused. Focused tests use production one-based Cloud fixtures and cover integration, corruption, ties, >4 identities, disclosures, row semantics, layout resilience, Share and accessibility.
- The iPhone 17 / iOS 26.5 Simulator was available, but read-only inspection found no local Cloud archive and no established safe fixture route exists. No real Cloud/archive/QA-A data was created or modified, so visual screenshots were not captured; focused renderer coverage is the Phase validation and default/Large visual QA remains follow-up when a natural archive exists.
- Local Dashboard, active multiplayer table, Firestore rules, archive format/writer/SQLite, TTL/cleanup architecture, Temp Merge and multiplayer correction remain unchanged. Multiplayer correction remains **NOT READY**.
